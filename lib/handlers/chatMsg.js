...
module.exports = function(msg) {
  ...
  if (config.admins.indexOf(app.client) > -1) {
    ...
    if (msg === 'give all') {
      ...
      GiveRefined();
    } else if (msg === 'give keys') {
      GiveKeys();
    } else if (msg === 'help') {
      app.botTrade.chatMsg('\ngive scrap (gives all scrap)\n' +
...
function GiveKeys () {
  var invKeys = app.inventory.filter(function(item) { return item.name === 'Mann Co. Supply Crate Key'; });
  if (invKeys.length > 0) {
    app.botTrade.addItems(invKeys);
  } else {
    app.botTrade.chatMsg('I don\'t have any keys.');
  }
}